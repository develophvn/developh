...
.wp-block-buttons.alignright .wp-block-button:first-of-type {
  margin-left: 0;
}
...
const FullScreenWrapper = ({ children, ...props }) => (
  <div {...props}>
    <style>{WpBlockStyles}</style>

    <style>
      {`
.bg-custom-template article,
.bg_block-template-default.single-bg_block article,
.page-template-templatepagecontent-only-php.page-template-content-only article,
...
.page-template.boldgrid-editor-template .bge-entry-content .the-content {
  width: 100%;
  max-width: 100%;
  padding: 0;
}
